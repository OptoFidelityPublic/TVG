...
:: Audio compression (select one)
:: - faac                AAC Advanced Audio Codec
:: - lamemp3enc          MP3
:: - ffenc_mp2           MP2
SET AUDIOCOMPRESSION=faac
...
gst-launch -q --gst-plugin-load=GstOFTVG.dll ^
	...
	! queue ! colorspace ! %COMPRESSION% ! %CONTAINER% name=mux ! filesink location=%OUTPUT% ^
	filesrc location=%INPUT% ! decodebin2 ! %AUDIOCOMPRESSION% ! mux.
